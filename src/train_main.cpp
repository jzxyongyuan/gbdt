...
void show_usage(const char* cmd) {
    printf("usage: %s -d conf -f file -i input -m model\n", cmd);
    return;
}
...
int main(int argc, char* argv[]) {
    ...
    ret = gbdt.train(g_conf.input_file);
    if (ret != 0) {
        LOG(WARNING) << "fail to train gbdt using file " << g_conf.input_file;
        return ret;
    }
    
    ret = gbdt.save(g_conf.model_file);
    if (ret != 0) {
        LOG(WARNING) << "fail to save gbdt to " << g_conf.model_file;
        return ret;
    }
